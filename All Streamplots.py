...
fig, ax = plt.subplots()
stream = ax.streamplot(X_rescale, Y_rescale, u_rescale, v_rescale, density = 1.5, color=v_mag, cmap=plt.cm.plasma, )
ax.set_title('Fluids in Blocked Channel')
plt.xlabel("Length along vessel (mm)")
plt.ylabel("Length across vessel (mm)")
colours = plt.colorbar(stream.lines)
colours.set_label("Velocities of fluid particles (mm/s)")
plt.show()
